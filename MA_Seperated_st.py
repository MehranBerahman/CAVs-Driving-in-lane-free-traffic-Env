# -*- coding: utf-8 -*-
"""
Created on Thu Jun  9 11:25:35 2022
...
class Critic(nn.Module):
    ...
    def __init__(self, state_size, action_size, agentsNum, seed,fc01_units=400,fc02_units=256, fc1_units=500, fc2_units=1024, fc3_units=500,
        ...
        self.bn4 = nn.BatchNorm1d(fc4_units)
        """
        self.fc01 = nn.Linear(state_size + action_size, fc01_units)
        self.fc02 = nn.Linear(fc01_units, fc02_units)

        self.fc1 = nn.Linear(state_size*(agentsNum-1) + action_size*(agentsNum-1), fc1_units)
        self.fc2j = nn.Linear(fc1_units+fc02_units, fc2_units)   
        self.fc3 = nn.Linear(fc2_units, fc3_units)
        self.fc4 = nn.Linear(fc3_units, fc4_units) 
        self.fc5 = nn.Linear(fc4_units, fc5_units) 
    ...
    def reset_parameters(self):
        ...
        self.fc02.weight.data.uniform_(*hidden_init(self.fc02))
        self.fc1.weight.data.uniform_(*hidden_init(self.fc1))
        self.fc2j.weight.data.uniform_(*hidden_init(self.fc2j))
        self.fc3.weight.data.uniform_(*hidden_init(self.fc3))
        self.fc4.weight.data.uniform_(*hidden_init(self.fc4))
        self.fc5.weight.data.uniform_(*hidden_init(self.fc5))
        self.fc6.weight.data.uniform_(-3e-3, 3e-3) 
        
    def forward(self, ob, own_act, R_states, R_actions):# states=state Dimention * agentsNum & actions=action dimention *agentsNum
        """Build a critic (value) network that maps (state, action) pairs -> Q-values.
        states = np.array(1,state_Dimention*agentsNum)
        actions = np.array(1,action_Dimention*agentsNum)
        """ 
        xr_sa = torch.cat((R_states, R_actions), dim=1)
        xr_sa = F.relu(self.fc1(xr_sa))
        xo = torch.cat((ob, own_act), dim=1)
        xo = F.relu(self.fc01(xo))
        xo = F.relu(self.fc02(xo))
        xj = torch.cat((xo, xr_sa), dim=1)
        """
...
class Agent():
    ...
    def __init__(self, state_size, action_size, agentsNum, random_seed, batch_size=256, device='cpu',
                 NOISE_DECAY_LIMIT=600,discount=0.99, tau=0.01, LR_ACTOR=1e-4, LR_CRITIC=1e-3, 
                 Weight_Decay=1e-3, BUFFER_SIZE=int(1e6) ):
        self.device = device 
...
class MADDPG_Sep():
    ...
    def learn(self, gamma, beta, tau):
        ...
        for agent_number, agent in enumerate(self.agents):
            ...
            # ---------------------------- update critic ---------------------------- #
            with torch.no_grad():
                ob, own_act, R_states, R_actions = self.StateAction(critic_next_state, Next_Action, agent, agent_number)
                Q_target_next = agent.critic_target(ob, own_act, R_states, R_actions)
            ...
            # Actual Q value based on reward rec'd at next step + future expected reward from Critic target network
            ob, own_act, R_states, R_actions = self.StateAction(actor_critic_state, actions, agent, agent_number)
            Q_expected = agent.critic_local(ob, own_act, R_states, R_actions)
            ...
            """
            # Compute actor loss
            ob, own_act, R_states, R_actions = self.StateAction(actor_critic_state, Mu_Action, agent, agent_number)
            actor_loss = agent.critic_local(ob, own_act, R_states, R_actions).reshape(-1) 
    ...
    def StateAction(self, actor_next_state, actions, agent, agent_number):
        first_state_index =agent.state_size * (agent_number)
        last_state_index = agent.state_size * (agent_number + 1) 
        logic_s = np.arange(first_state_index, last_state_index)
        R_states = actor_next_state[:,[i not in logic_s for i in np.arange(self.total_state_size)]]
        ob = actor_next_state[:,first_state_index:last_state_index]
        first_act_ind, last_act_ind = agent.action_size* (agent_number), agent.action_size* (agent_number+1)
        logic_a = np.arange(first_act_ind, last_act_ind)
        own_act = actions[:,first_act_ind:last_act_ind]
        R_actions = actions[:,[i not in logic_a for i in np.arange(self.total_action_size)]]
        return ob, own_act, R_states, R_actions    
